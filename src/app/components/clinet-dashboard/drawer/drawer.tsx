...
import { Drawer, DrawerContent, DrawerHeader, DrawerBody, DrawerFooter, Divider, Button, cn } from "@nextui-org/react";
...
function ClientDrawer({ onPress, isOpen, onOpenChange, name, logout }) {
    ...
    <>
      ...
      <Drawer isOpen={isOpen} onOpenChange={onOpenChange} placement="left" size="xs">
        <DrawerContent>
          {(onClose) => (
            <>
              <DrawerHeader className="flex flex-col gap-1">{name}</DrawerHeader>
              <DrawerBody>
                <ListItem />
              </DrawerBody>
              <Divider />
...
export default connect(mapStateToProps, mapDispatchToProps)(ClientDrawer);
